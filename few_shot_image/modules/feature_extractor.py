...
class FeatEmbedding(object):
    def __init__(self, config, hidden_num, channels, conv_initializer, k=5):
        self.config = config
        self.hidden_num = hidden_num  # number of filters
    ...
    def model(self, images):
        ...
        with tf.variable_scope('local4', reuse=tf.AUTO_REUSE):
            local4_weight = tf.get_variable(name='weight', shape=[384, self.config['embed_dim']],
                                            initializer=tf.truncated_normal_initializer(stddev=0.04))
            local4_biases = tf.get_variable(name='biases', shape=[self.config['embed_dim']],
